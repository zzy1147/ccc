...
//        }
//        }
    //请求权限后回调的方法 //参数： requestCode  是我们自己定义的权限请求码 //参数： permissions  是我们请求的权限名称数组 //参数： grantResults 是我们在弹出页面后是否允许权限的标识数组，数组的长度对应的是权限名称数组的长度，数组的数据0表示允许权限，-1表示我们点击了禁止权限
        ...
        if (mRequestCode == requestCode) {
            ...
            if (hasPermissionDismiss) {
                if (showSystemSetting) {
                    ...
                    alertdialog_builder.setTitle("您禁用了一些权限,您确定要这样吗").setNegativeButton("取消", new DialogInterface.OnClickListener() {
                        @Override
